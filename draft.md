...
**Alignment training penalizes idiosyncrasy.** RLHF reward models are trained on human preferences for responses that are helpful, harmless, and honest in general, not for any particular evaluator. The result is convergence toward a consensus persona: agreeable, qualified, eager to help. This is not individuality. It is the statistical ghost of every preference annotator, averaged into a single compliant voice. Existing alignment training, which moves partially in the Y direction by constraining model behavior to follow specified norms, already shows a measurable cost to general capability, a phenomenon documented as the "alignment tax" [25]. If even partial behavioral constraint degrades X-axis performance, explicit identity optimization, which would impose far stronger constraints, should be expected to show the same pattern for the same architectural reasons. Systems trained this way exhibit well-documented sycophancy, abandoning stated positions to agree with users [4], which is the expected behavior of a system optimized to lack structural commitments.
...
[21] (Authors unlisted). Agent drift: quantifying behavioral degradation in multi-agent LLM systems over extended interactions. arXiv:2601.04170 (2026).
